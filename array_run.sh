                  ...
                  for model_name in "${model_name_options[@]}"; do
                    for parser_residual in "${parser_residual_options[@]}"; do
                      for use_lora in "${use_lora_options[@]}"; do
                        ...
                        valid_combinations+=("$seed $parser_type $freeze_encoder $gnn_enc_layers $arc_norm $use_parser_rnn $use_tag_embeddings_in_parser $parser_rnn_type $model_name $parser_residual $use_lora")
                      done
                    done
                  done
                done
...
if [ -n "$SLURM_ARRAY_TASK_ID" ]; then
  if [ "$SLURM_ARRAY_TASK_ID" -lt "$total_combinations" ]; then
    ...
    read -r seed parser_type freeze_encoder gnn_enc_layers arc_norm use_parser_rnn \
         use_tag_embeddings_in_parser parser_rnn_type model_name parser_residual use_lora <<< "$current_combination"
