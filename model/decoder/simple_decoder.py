...
class SimpleDecoder(nn.Module):
    ...
    def forward(self,
        ...
        if self.training or not self.use_mst_decoding_for_validation:
            # _greedy_decode will simply take the .max(dim=-1)
            # on the predictions of the edgeRNN
            # which will directly predict the head_tag_logits
            # which means an arc will be considered present
            # if the head_tag is not class 0 (i.e. no edge label)
