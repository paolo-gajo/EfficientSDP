...
class TriParser(nn.Module):
    def __init__(
        ...
        self._dropout = nn.Dropout(self.config['mlp_dropout'])
    ...
    def forward(
        ...
        **kwargs,
        ...
        sib_head = self._dropout(F.elu(self.sib_head_ff(input)))
        sib_dep = self._dropout(F.elu(self.sib_dep_ff(input)))
        cop_head = self._dropout(F.elu(self.cop_head_ff(input)))
        cop_dep = self._dropout(F.elu(self.cop_dep_ff(input)))
        gp_head = self._dropout(F.elu(self.gp_head_ff(input)))
        gp_dep = self._dropout(F.elu(self.gp_dep_ff(input)))
        gp_head_dep = self._dropout(F.elu(self.gp_head_dep_ff(input)))
