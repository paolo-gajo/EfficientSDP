...
class GNNParser(nn.Module):
    def __init__(
        ...
        self.dept_tag_feedforward = nn.Linear(encoder_dim, arc_representation_dim)
    ...
    def forward(
        ...
        if self.config["arc_pred"] == "attn":
            ...
            if self.config['use_parser_gnn']:
                ...
                for i, b in enumerate(attended_arcs):
                    ...
                    dept_arc_reps.append(self.gnn(dept_arc_representation[i], arc_edge_index))
                head_arc_representation = torch.stack(head_arc_reps, dim = 0)
                dept_arc_representation = torch.stack(dept_arc_reps, dim = 0)
        ...
        if self.training or not self.use_mst_decoding_for_validation:
            predicted_heads, predicted_head_tags = self._greedy_decode(
                head_tag_representation, dept_tag_representation, attended_arcs, mask
            )
        else:
            predicted_heads, predicted_head_tags = self._mst_decode(
                head_tag_representation,
                dept_tag_representation,
                attended_arcs,
                og_mask,
            )
        ...
        if head_indices is not None and head_tags is not None:
            ...
            arc_nll, tag_nll = self._construct_loss(
                ...
                dept_tag_representation=dept_tag_representation,
    ...
    def _construct_loss(
        ...
        dept_tag_representation: torch.Tensor,
        ...
        head_tag_representation : ``torch.Tensor``, required.
            ...
            for the given arcs.
        dept_tag_representation : ``torch.Tensor``, required
            A tensor of shape (batch_size, sequence_length, tag_representation_dim),
            which will be used to generate predictions for the dependency tags
            for the given arcs.
        ...
        head_tag_logits = self._get_head_tags(
            head_tag_representation, dept_tag_representation, head_indices
        )
        # if not self.config['use_step_mask']:
    ...
    def _greedy_decode(
        ...
        # shape (batch_size, sequence_length, num_head_tags)
        head_tag_logits = self._get_head_tags(
            head_tag_representation, dept_tag_representation, heads
        )
    ...
    def _mst_decode(
        ...
        dept_tag_representation = dept_tag_representation.unsqueeze(1)
        dept_tag_representation = dept_tag_representation.expand(
            *expanded_shape
        ).contiguous()
        # Shape (batch_size, sequence_length, sequence_length, num_head_tags)
        pairwise_head_logits = self.tag_bilinear(
            head_tag_representation, dept_tag_representation
        )
    ...
    def _get_head_tags(
        ...
        head_tag_logits = self.tag_bilinear(
            selected_head_tag_representations, dept_tag_representation
        )
